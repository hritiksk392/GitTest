...
unordered_set <string> kw =  {"int","if","else","for","while","break","string","return","main","print","input","char","bool","true","false","until"};
		...
		switch(state){
			...
			case 4:
			 ...
			 if(ch=='='){
			 	...
			 	cout <<"Token "<<symbolTable[word]<<" String ! (NEGATION),line number "<<lineNo<<endl;
			 	fseek(f,-1,SEEK_CUR);
			 }
			break;
...
void arithOperatorDFA(){
	...
	switch(ch){
		...
		break;
		case ',':
		cout <<"Token "<<symbolTable[word]<<" comma ',' ,line number "<<lineNo<<endl;
		break;
		case '&':
		ch= fgetc(f);
		if(ch=='&') {
